...
% @TBM
DATA_PATH_IMAGE = [ '../2nd_session/undist_images/' tool '/subImages' ];
...
for noiseMag=noiseMagnitudes
    ...
    for iError=1 % Change this line when using noise on the shaft radius measurement, e.g. 1:length(errorRadius)
        ...
        for iFrameToProcess = indicesToProcess
            ...
            % @TBM
            if flags.doSave
                save(['../2nd_session/undist_images/' tool '/subImages/results' 'Radius' int2str(iError) '_noise_' noiseMag '.mat'],'refinePoseToSave');
            end
        end
    end
end
